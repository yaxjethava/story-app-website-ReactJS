  ...
  return (
    <div className="container py-5">
      ...
      <div className="row">
        ...
        {stories.map(story => (
          <div className="col-md-4 mb-4 cardBG" key={story.id}>
            <div className="card h-100 shadow-sm story-card">
              <img src={story.image} className="card-img-top" alt={story.title} />
              <div className="card-body">
                <h5 className="card-title">{story.title}</h5>
                <p className="card-text">{story.excerpt}</p>
                <Link to={`/story/${story.id}`} className="btn btn-warning fw-bold btn-sm">Read More</Link>
              </div>
            </div>
          </div>
        ))}
